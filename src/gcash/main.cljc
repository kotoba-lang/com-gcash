(ns gcash.main
  "Kotodama WASM entrypoint for the Gcash clean-room actor (L4) — Clojure port.
  ...
  facts (`gcash.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"gcash.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "gcash-compat" :tier tier :entities entities} 200])
